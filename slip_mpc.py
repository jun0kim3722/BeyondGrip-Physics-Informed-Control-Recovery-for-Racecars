...
def main():
    ...
    env = assettoCorsa.make_ac_env(
        ...
        env_kwargs=dict(                     
            ...
            recovery_time=0.5, # time in seconds
        )
    )
    mpc = MPC_controller(config, env.norm_steer_at_max, target_speed=50)
    slip_init = SlipInitializer(env, mpc)

    # Attach slip initializer into env
    env.slip_initializer = slip_init
    ...
    agent = Agent(env=env, test_env=copy.deepcopy(env), algo=algo, log_dir=config.work_dir,
